classdef camera_pixelfly < handle
    properties (Access = public)
        mode
        vid         % video object
        src         % record object
        pixelfly_cam = 'pcocameraadaptor_r2019b';  % to-do: create settings
    end
    methods
        function obj = camera_pixelfly(cam_mode, exposure)
            hw_status = imaqhwinfo;
            if ismember(obj.pixelfly_cam, hw_status.InstalledAdaptors)
                obj.mode = cam_mode;
                obj.vid = videoinput(obj.pixelfly_cam, 1, obj.mode);
